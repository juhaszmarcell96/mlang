...
Token::Token () : m_type(token_types::none) {}
void Token::set_type (token_types type) { m_type = type; }
void Token::append (char ch) { m_value.push_back(ch); }
void Token::clear() {
...
const std::string& Token::get_value () const { return m_value; }
const token_types Token::get_type () const { return m_type; }

void Token::set_line (std::size_t line) { m_line = line; }
void Token::set_pos (std::size_t pos) { m_pos = pos; }

std::size_t Token::get_line () const { return m_line; }
std::size_t Token::get_pos () const  { return m_pos; }
...
void Tokenizer::start_new_token (token_types type) {
    m_current_token.clear();
    m_current_token.set_line(m_code.get_line_num());
    m_current_token.set_pos(m_code.get_column());
...
void Tokenizer::finalize_token (token_types type) {
    //std::cout << "finalizing token " << m_current_token.get_value() << " position " << m_current_token.get_line() << ":" << m_current_token.get_pos() << std::endl;
    m_current_token.set_type(type);
    m_tokens.push_back(m_current_token);
    m_current_token.clear();
...
void Tokenizer::append_char (char c) {
    m_current_token.append(c);
}

void Tokenizer::set_state (tokenizer_states state) {
...
void Tokenizer::process_double_quotes () {
    if (m_current_state == tokenizer_states::none) {
        /* start a string */
...
void Tokenizer::process_standalone_sign (token_types type) {
    if (m_current_state == tokenizer_states::none) {
...
Tokenizer::Tokenizer (const std::string& script) : m_code(script) {}

const std::vector<Token>& Tokenizer::get_tokens () const { return m_tokens; }
void Tokenizer::clear () { m_tokens.clear(); }
